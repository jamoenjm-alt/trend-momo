...
- `property-board.html` — AU property trend/momentum board (same architecture: single
  ...
  board. Stability = All-Signals regime at 0/−13/−26/−39 weeks. Regime labels on BOTH boards
  are Strong/Weak Uptrend · Sideways · Weak/Strong Downtrend (internal keys stay
  STRONG_BULL etc. for back-compat — don't rename the keys or CSS classes).
...
- `update-property.py` — bakes `data/property.json` from SQM Research free chart pages
  ...
  `?region=<slug>&type=c`, towns canonical `?postcode=NNNN`, plus `national=1` / `avg=1`.
  5 pages per region (prices/rents/yield weekly; vacancy/stock monthly), 0.5s between
  requests, atomic write, failed regions keep previous bake. SQM data is personal-reference
  only — keep attribution, don't resell.
- `.github/workflows/update-property.yml` — weekly bake Tue 16:00 UTC (Wed 2am AEST; SQM
  publishes Tuesdays), commits `data/property.json`.
